...
class cfg:
    ...
    The default step sizes are 1, 5, 2, 2.5, 4, and 3.
...
class TicksGenerator:
    ...
    def ticks(self, dmin, dmax, axis_length, axis_horizontal):
        ...
        - `axis_horizontal`: whether the axis is oriented horizontally
          (or vertically)
...
class Ticks:
    ...
    def __init__(self, amin, amax, decimal_values,
        ...
        self.amin = amin
        "axis lower bound, `float`"

        self.amax = amax
        "axis upper bound, `float`"

        self.values = [float(dv) for dv in decimal_values]
        "tick values, `list` of `float`s"

        self.decimal_values = decimal_values
        "exact tick values, `list` of `decimal.Decimal`s"

        self.labels = labels
        "tick labels, `list` of `str`"

        self.plabel = plabel
        """
        power label, `str` or `None`
        ...
        "tick label font size, in TeX pt (2.54 cm / 72.27), `float`"
